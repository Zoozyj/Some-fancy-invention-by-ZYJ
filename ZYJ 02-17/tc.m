...
function tc = tc(index)
    ...
    t_g.env_fix.Sig_int_star=t_g.nr.prs_symbols(1) * t_g.nr.N_fft ...
                     + sum(t_g.nr.N_s_cp_ofdm(1:t_g.nr.prs_symbols(1)))+1; % The interested sample range
    t_g.env_fix.Sig_int_end =(t_g.nr.prs_symbols(end)+1) * t_g.nr.N_fft ...
    ...
    % Channel setting
    t_g.env_fix.ch_mode='Pro'
    t_g.env_fix.ch = 'cdl-c';                                              % channel model to use
