function [signalOut,pathGains,sampleTimes] = nrCDLChannel1(signalIn,channel)
...
% TODO:
% Need to add pathloss and shadow fading.
% The 3D distance seems not passed to this function. APPLY EXTERNALLY?
% DELAYS NEED TO BE SCALED PROPERLY TO MATCH GEOMETRY.
% Figure out a nice to specify individual antenna patterns beyond those in
% the Matlab NR toolbox. Perhaps a function callback.
% How do we take into account that AoA/AoD vectors have opposite sign?
% Check if any of the additional modeling components described in TR 38.901, Sec. 7.6 are needed.
% The scaling of the angles gives only approximations to the desired mean
% and angular spreads, not exact values. Check if the implementaiton is
% correct.
...
txArrayDefault.Size = [4 8 1 1 1];%[2 2 2 1 1]; % Given as [M N P Mg Ng].
txArrayDefault.ElementSpacing = [0.5 0.5 0.0 0.0];%[0.5 0.5 1.0 1.0]; % Given as [dV dH dgV dgH].
...
txArrayDefault.Element = '38.901'; % 'isotropic'|'38.901'|'38.855 model 1' in TR 38.901 7.3 and TR 38.855 6.1.
...
rxArrayDefault.Element = 'isotropic'; % 'isotropic'|'38.901'|'38.855 model 1' in TR 38.901 7.3 and TR 38.855 6.1.
...
switch lower(channel.DelayProfile)
    case 'cdl-a'
        ...
        channel.AngleSpreads = [5 11 3 3];
        channel.XPR = 10;
        channel.HasLOSCluster = false;
        %channel.DelaySpread = ;
        %channel.KFactorScaling = ;
        %channel.ClusterDelaySpread = ;
    case 'cdl-b'
        ...
        channel.AngleSpreads = [10 22 3 7];
        channel.XPR = 8;
        channel.HasLOSCluster = false;
        %channel.DelaySpread = ;
        %channel.KFactorScaling = ;
...
% Coefficient computation and channel filtering. First compute
% dimensionality of input/output signals.
Ns = channel.NumTimeSamples;
[Nr,Nt,Np] = size(nrCDLChannelCoeff(channel,0));
% Then, sanity check input signal...
...
% ... and allocate array for filtered signal.
...
for timeSampleIndex = 1:Ns
    ...
    if (mod(timeSampleIndex-1,floor(channel.SampleRate/Fcg)) == 0)
        if (timeSampleIndex == 1)
            [coeff,rays] = nrCDLChannelCoeff(channel,sampleTime);
        else
...
% We divide the implementation i several parts. In one part, we compute
...
function [coeff,rays] = nrCDLChannelCoeff(channel,t,rays)
...
if (nargin<3)
    rays.AnglesAoD = (repmat(channel.AnglesAoD,numRays,1) + channel.AngleSpreads(1)*repmat(IntraClusterOffsetAngles(:),1,numClusters)).';
    rays.AnglesAoA = (repmat(channel.AnglesAoA,numRays,1) + channel.AngleSpreads(2)*repmat(IntraClusterOffsetAngles(:),1,numClusters)).';
    rays.AnglesZoD = (repmat(channel.AnglesZoD,numRays,1) + channel.AngleSpreads(3)*repmat(IntraClusterOffsetAngles(:),1,numClusters)).';
    rays.AnglesZoA = (repmat(channel.AnglesZoA,numRays,1) + channel.AngleSpreads(4)*repmat(IntraClusterOffsetAngles(:),1,numClusters)).';
end
...
H = zeros(numRx,numTx,numClusters);
for u = 1:numRx
    for s = 1:numTx
        for n = channel.HasLOSCluster+1:numClusters % Treat all clusters as weak clusters.
            for m = 1:numRays
                H(u,s,n) =...
                    H(u,s,n) +...
                    sqrt(dB2pow(channel.AveragePathGains(n))/numRays)*...
                    panelResponse(channel.ReceiveAntennaArray,rays.AnglesAoA(n,m),rays.AnglesZoA(n,m)).'*...
                    (exp(1i*squeeze(rays.Phases(n,m,:,:))).*[1 1/sqrt(rays.kappa); 1/sqrt(rays.kappa) 1])*...
                    panelResponse(channel.TransmitAntennaArray,rays.AnglesAoD(n,m),rays.AnglesZoD(n,m))*...
                    rayPhaseCenterOffset(channel.ReceiveAntennaArray,u,rays.AnglesAoA(n,m),rays.AnglesZoA(n,m))*...
                    rayPhaseCenterOffset(channel.TransmitAntennaArray,s,rays.AnglesAoD(n,m),rays.AnglesZoD(n,m))*...
                    rayDopplerPhaseOffset(channel,rays.AnglesAoA(n,m),rays.AnglesZoA(n,m),t);
            end
        end        
    end
end
    ...
    for u = 1:numRx
        for s = 1:numTx
            ...
            H(u,s,1) =...
                ...
                panelResponse(channel.ReceiveAntennaArray,channel.AnglesAoA(1),channel.AnglesZoA(1)).'*...
                [1 0;0 -1]*...
                panelResponse(channel.TransmitAntennaArray,channel.AnglesAoD(1),channel.AnglesZoD(1))*...
...
% function y = rayPhaseCenterOffset(antennaArray,panelIndex,angleAzimuth,angleZenith)
% % Here, panelLoc is defined in TR38.901, Sec. 7.5 as the location vector of
% % panel panelIndex for panelIndex = 1,...,Mg*Ng.
% Mg = antennaArray.Size(4);
% panelHorLoc = floor((panelIndex-1)/Mg);
% panelVerLoc = mod(panelIndex-1,Mg);
% dgV = antennaArray.ElementSpacing(3);
% dgH = antennaArray.ElementSpacing(4);
% panelLoc = [dgH*panelHorLoc 0 dgV*panelVerLoc].'; % Obs! lambda0 is cancelled out.
% rayDir = [sin(deg2rad(angleZenith))*cos(deg2rad(angleAzimuth)),sin(deg2rad(angleZenith))*sin(deg2rad(angleAzimuth)),cos(deg2rad(angleZenith))].';
% y = exp(1i*2*pi*rayDir.'*panelLoc);
...
function F = panelResponse(antennaArray,phiGCS,thetaGCS)
% The implementation of this function is based on TR38.901 Sec. 7.1 and
% Sec. 7.3. Our goal is to implement (7.1-11). For that, we first compute
% (phiLCS,thetaLCS) from (phiGCS,thetaGCS) by using equaitons (7.1-7) and
% (7.1-8).
[phiLCS,thetaLCS,R] = GCS2LCS(phiGCS,thetaGCS,antennaArray.Orientation);

% Next, compute the polarized field components in the LCS. Note that we
% need to account for polarization angles as specified in TR 38.901, Sec.
% 7.3.2.
% Obs, here we assume that must have 
% antennaArray.PolarizationAngles(2) = antennaArray.PolarizationAngles(1) + 90.
if (antennaArray.PolarizationAngles(2) ~= antennaArray.PolarizationAngles(1) + 90)
    disp('WARNING: Assumption on polatization angles violated.');
end
zeta = antennaArray.PolarizationAngles(1); 
if (strcmpi(antennaArray.PolarizationModel,'Model-1'))
    % Obs, we use (7.3-3) in TR 38.901, but not the expressions for
    % cos(Psi) and sin(Psi) immediately below.
    FLCS = singleResponse(phiLCS,thetaLCS,antennaArray.Element);
    numPols = antennaArray.Size(3);
    if (numPols == 1)
        FLCS = [cos(deg2rad(zeta)) -sin(deg2rad(zeta)); sin(deg2rad(zeta)) cos(deg2rad(zeta))]*sqrt(FLCS)*[1, 0].';
    else
        FLCS = [cos(deg2rad(zeta)) -sin(deg2rad(zeta)); sin(deg2rad(zeta)) cos(deg2rad(zeta))]*sqrt(FLCS)*[1, 1].'/sqrt(2);
    end
else
    % We use (7.3-4) and (7.3-5) in TR 38.901.
    FLCS = singleResponse(phiLCS,thetaLCS,antennaArray.Element);
    FLCS = [cos(deg2rad(zeta)), sin(deg2rad(zeta))].'*sqrt(FLCS);
end    

% Finally, compute the polarized field components in the GCS using
% (7.1-11). We actually use (7.1-9), from which (7.1-11) is derived.
thetaGCSDir = [cos(deg2rad(thetaGCS))*cos(deg2rad(phiGCS)), cos(deg2rad(thetaGCS))*sin(deg2rad(phiGCS)), -sin(deg2rad(thetaGCS))].'; % d\hat(r)/d\theta / 1.
phiGCSDir = [-sin(deg2rad(phiGCS)), cos(deg2rad(phiGCS)), 0].'; % d\hat(r)/d\phi / sin(\theta).
thetaLCSDir = [cos(deg2rad(thetaLCS))*cos(deg2rad(phiLCS)), cos(deg2rad(thetaLCS))*sin(deg2rad(phiLCS)), -sin(deg2rad(thetaLCS))].'; % d\hat(r)/d\theta / 1.
phiLCSDir = [-sin(deg2rad(phiLCS)), cos(deg2rad(phiLCS)), 0].'; % d\hat(r)/d\phi / sin(\theta).
F = [thetaGCSDir phiGCSDir].'*R*[thetaLCSDir phiLCSDir]*FLCS;

function F = singleResponse(phiLCS,thetaLCS,elementType)
% Obs! Are we using the dBi feature correctly?
% We consider 3 types of antenna responses for a single antenna element.
switch lower(elementType)
    case 'isotropic'
        F = 1;
    case '38.901' % TR 38.901, Table 7.3-1.
        theta3dB = 65; % In degrees.
        SLAv = 30; % In dB.
        Av = -min(12*((thetaLCS-90)/theta3dB)^2,SLAv);
        phi3dB = 65; % In degrees.
        Amax = 30; % In dB.
        Ah = -min(12*(phiLCS/phi3dB)^2,Amax);
        GEmax = 8; % In dBi.
        F = db2pow(-min(-(Ah+Av),Amax) + GEmax);
    case '38.855 model 1' % TR 38.855, Table 6.1.1-2.        
        theta3dB = 90; % In degrees.
        SLAv = 25; % In dB.
        Av = -min(12*((thetaLCS-90)/theta3dB)^2,SLAv);
        phi3dB = 90; % In degrees.
        Amax = 25; % In dB.
        Ah = -min(12*(phiLCS/phi3dB)^2,Amax);
        GEmax = 5; % In dBi.
        F = db2pow(-min(-floor(Ah+Av),Amax) + GEmax);
    otherwise
        disp('Unkown elementType');
end
...
numRays = length(IntraClusterOffsetAngles);
numClusters = length(channel.AnglesAoD);
anglesAoD = repmat(channel.AnglesAoD,numRays,1) + channel.AngleSpreads(1)*repmat(IntraClusterOffsetAngles(:),1,numClusters);
anglesAoA = repmat(channel.AnglesAoA,numRays,1) + channel.AngleSpreads(2)*repmat(IntraClusterOffsetAngles(:),1,numClusters);
anglesZoD = repmat(channel.AnglesZoD,numRays,1) + channel.AngleSpreads(3)*repmat(IntraClusterOffsetAngles(:),1,numClusters);
anglesZoA = repmat(channel.AnglesZoA,numRays,1) + channel.AngleSpreads(4)*repmat(IntraClusterOffsetAngles(:),1,numClusters);
...
function y = ClipBetween0And180(x)
% Clip x to [0,180].
y = x;
y(y<0) = 0;
y(y>180) = 180;
...
function [phiLCS,thetaLCS,R] = GCS2LCS(phiGCS,thetaGCS,orientation)
[X, Y, Z] = sph2cart(deg2rad(phiGCS),deg2rad(90-thetaGCS),1);
cartGCS  = [X Y Z].';
bearing = orientation(1);
downtilt = orientation(2);
slant = orientation(3);
Ralpha = [cos(deg2rad(bearing)) -sin(deg2rad(bearing)) 0; sin(deg2rad(bearing)) cos(deg2rad(bearing)) 0; 0 0 1];
Rbeta = [cos(deg2rad(downtilt)) 0 sin(deg2rad(downtilt)); 0 1 0; -sin(deg2rad(downtilt)) 0 cos(deg2rad(downtilt))];
Rgamma = [1 0 0; 0 cos(deg2rad(slant)) -sin(deg2rad(slant)); 0 sin(deg2rad(slant)) cos(deg2rad(slant))];
R = Ralpha*Rbeta*Rgamma;
Rinv = R.';
cartLCS = Rinv*cartGCS;
[phiLCS,thetaLCS,~] = cart2sph(cartLCS(1),cartLCS(2),cartLCS(3));
phiLCS = WrapToPlusMinus180(rad2deg(phiLCS));
thetaLCS = 90 - rad2deg(thetaLCS);
if (thetaLCS<0 || thetaLCS>180)
